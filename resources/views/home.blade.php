...
<script>
    $(function(){
        ...
        $(".btnDeselect").on("click", function(){
            ...
            $.ajax(
            {
                url: "{{ route('oportunidades.descandidatarse') }}",
                type: 'GET',
                data: 
                { 
                ...
                success: function (response) {
                    ...
                    desmarcarTables(table, idOportunidade);
                },
                complete: function (){
                    $("#loading").hide(); 
                }
        ...
        function desmarcarTables(table ,idOportunidade){
            var indexes = [];
            table.rows( function ( idx, data, node ) {             
                if(data[0] === idOportunidade.toString()){
                    indexes.push(node);                  
            ...
            for(var idx in indexes){
                $(indexes[idx]).removeClass('selected');
                $($(indexes[idx]).find('button')).hide();
                $($(indexes[idx]).find('.btnGrid')).show();
            }
        }
    });
</script>
